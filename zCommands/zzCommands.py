...
class Levels():
  ...
  async def make_user_rank_card_just_for_showing_them_lol(userid: str, user_name: str, avatar: str):
    with open("jsons/userdata.json", "r") as f:
        user_data = json.load(f)
    xp = 3250
    lvl = 78
    ...
    print(f"https://cdn.discordapp.com/avatars/{str(userid)}/{str(avatar)}.png")

    ## Rank card
    background = Editor(f"images/{image}.png")
    profile = await load_image_async(f"https://cdn.discordapp.com/avatars/{str(userid)}/{str(avatar)}.png")
...
class Economy():
  ...
  def update_bank_using_id(userid: str, change = 0, mode="wallet"):
    ...
    return ([users[str(userid)]['wallet'],users[str(userid)]['bank']])
...
class General():
  ...
  def chance(percent: str, YN_format= True):
    percent = int(percent.replace("%", ""))
    random_num = random.randint(0, 10000)
    num = int((percent/100)*10000)

    if random_num >= num:
      return "Yes" if YN_format else 1
    else:
      return "No" if YN_format else 0
...
class Time():
  ...
  def add_time(now: str, addTime: str):
    new_time = now+addTime
    return new_time
...
class Dashboard():
  ...
  async def make_user_rank_card(userid=None, username=None, avatar=None, card=None, blend=None, text_color=None, bar_color=None, normal=False):
    if normal == True:
      pass
    else:

      ###################### from here not normal
      ...
      background.bar(
          ...
          height=40,
          percentage=percentage,
          fill=f"#{str(bar_color)}",
          radius=20,
      )
      background.text((200, 40), str(username), font=poppins, color=f"#{str(text_color)}")
  
      background.rectangle((200, 100), width=350, height=2, fill=f"#{str(bar_color)}")
      background.text(
          (200, 130),
          f"Level : {lvl}   "
          + f" XP : {xp} / {(lvl+1) * 100}",
          font=poppins_small,
          color=f"#{str(text_color)}",
      )
      background.save(fp="./")
      
      return "ok"

  async def hmmm(userid=None, username=None, avatar=None, card=None, blend=None, text_color=None, bar_color=None, normal=False):
    if normal:
      pass
    else:
      xp = 3250
      ...
      rank_card = Image.open(f"images/{str(card)}.png")
      rank_card.convert("RGBA")
      
      _bytes = BytesIO(requests.get(f"https://cdn.discordapp.com/avatars/{str(userid)}/{str(avatar)}").content)
      user_profile = Image.open(_bytes).convert("RGBA")


      #user profile stuff
      width, height = user_profile.size
      ...
      user_profile = user_profile.crop(resize).resize(
          (ideal_width, ideal_height), Image.ANTIALIAS
      )

      #making circle image
      background = Image.new("RGBA", size=user_profile.size, color=(255, 255, 255, 0))
      holder = Image.new("RGBA", size=user_profile.size, color=(255, 255, 255, 0))
      mask = Image.new("RGBA", size=user_profile.size, color=(255, 255, 255, 0))
      mask_draw = ImageDraw.Draw(mask)
      mask_draw.ellipse((0, 0) + user_profile.size, fill="black")
      holder.paste(user_profile, (0, 0))
      user_profile = Image.composite(holder, background, mask)

      if str(blend) == "1":
        blend_image = Image.open("images/anothertry.png")
        rank_card = Image.blend(rank_card, blend_image, alpha=.5)

      blank = Image.new("RGBA", size=rank_card.size, color=(255, 255, 255, 0))

      blank.paste(user_profile, (30, 30))
      rank_card = Image.alpha_composite(rank_card, blank)
